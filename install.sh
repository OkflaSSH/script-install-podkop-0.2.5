...
main() {
    ...
    while true; do
        ...
        case $INSTALL_DOH in
            y|Y)
                info "Установка https-dns-proxy и русского перевода..."
                opkg install luci-app-https-dns-proxy luci-i18n-https-dns-proxy-ru
                if [ $? -ne 0 ]; then
                    printf "\033[33;1mПредупреждение: не удалось установить https-dns-proxy. Возможно, пакеты недоступны в вашем репозитории.\033[0m\n"
                else
                    info "Пакеты https-dns-proxy успешно установлены."
                fi
                break
                ;;
            n|N)
