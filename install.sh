...
fail() {
    printf "\033[31;1mОшибка: %s\033[0m\n" "$1" >&2
    exit 1
}
...
main() {
    ...
    info "Загрузка пакетов Podkop версии ${PODKOP_VERSION}..."
    ...
    info "Обновление списка пакетов (opkg update)..."
    opkg update

    if opkg list-installed | grep -q dnsmasq-full; then
        echo "dnsmasq-full уже установлен."
    else
        info "Установка dnsmasq-full..."
    ...
    if [ "$openwrt_release" -ge 24 ]; then
        if uci get dhcp.@dnsmasq[0].confdir 2>/dev/null | grep -q '/tmp/dnsmasq.d'; then
            ...
            info "Настройка опции confdir для OpenWrt 24+..."
    ...
    if [ -f "/etc/init.d/podkop" ]; then
        ...
        info "Установка podkop..."
        add_tunnel
    fi

    info "Установка основных пакетов..."
    ...
    printf -- "------------------------------------------------------\n"
    echo "Установить DNS-over-HTTPS прокси (luci-app-https-dns-proxy)?"
    echo "Это позволит шифровать ваши DNS-запросы для повышения приватности."
    echo "Рекомендуется, если вы не используете VPN/прокси для всего трафика."
    while true; do
        read -r -p 'Ваш выбор (y/n): ' INSTALL_DOH
        case $INSTALL_DOH in
            y|Y)
                info "Установка luci-app-https-dns-proxy..."
                opkg install luci-app-https-dns-proxy
                
                # Проверяем, удачно ли установился основной пакет
                if [ $? -eq 0 ]; then
                    info "Пакет luci-app-https-dns-proxy успешно установлен."
                    # Если да, спрашиваем про перевод
                    echo "Установить русский перевод для https-dns-proxy? (y/n)"
                    while true; do
                        read -r -p 'Ваш выбор: ' INSTALL_DOH_RU
                        case $INSTALL_DOH_RU in
                            y|Y)
                                info "Установка русского перевода..."
                                opkg install luci-i18n-https-dns-proxy-ru
                                break
                                ;;
                            n|N)
                                echo "Установка перевода пропущена."
                                break
                                ;;
                            *)
                                echo "Пожалуйста, введите y или n."
                    ...
                    printf "\033[33;1mПредупреждение: не удалось установить https-dns-proxy. Возможно, пакет недоступен в вашем репозитории.\033[0m\n"
                fi
                break
                ;;
            n|N)
                echo "Установка https-dns-proxy пропущена."
                break
                ;;
            *)
                echo "Пожалуйста, введите y или n."
...
install_awg_packages() {
    ...
    for pkg_name in $packages; do
        ...
        info "Загрузка ${pkg_name}..."
        wget -q -O "$AWG_DIR/$PKG_FILENAME" "$DOWNLOAD_URL"
        [ $? -ne 0 ] && fail "Не удалось скачать ${pkg_name}. Установите его вручную и запустите скрипт снова."

        info "Установка ${pkg_name}..."
        ...
        info "${pkg_name} успешно установлен."
    done
    rm -rf "$AWG_DIR"
}
...
wg_awg_setup() {
    PROTOCOL_NAME=$1
    info "Настройка ${PROTOCOL_NAME}"
    if [ "$PROTOCOL_NAME" = 'Wireguard' ]; then
        INTERFACE_NAME="wg0"; CONFIG_NAME="wireguard_wg0"; PROTO="wireguard"; ZONE_NAME="wg"
    fi
    if [ "$PROTOCOL_NAME" = 'AmneziaWG' ]; then
        INTERFACE_NAME="awg0"; CONFIG_NAME="amneziawg_awg0"; PROTO="amneziawg"; ZONE_NAME="awg"
        echo "Использовать конфиг AmneziaWG или базовый конфиг Wireguard + авто-обфускация?"
        echo "1) AmneziaWG"; echo "2) Wireguard + авто-обфускация"; read -r CONFIG_TYPE
    fi
    ...
    while true; do
        ...
        if echo "$WG_IP" | grep -Eq '^([0-9]{1,3}\.){3}[0-9]{1,3}/[0-9]+$'; then break; else echo "Неверный формат IP-адреса."; fi
    done
    ...
    if [ "$PROTOCOL_NAME" = 'AmneziaWG' ]; then
        if [ "$CONFIG_TYPE" = '1' ]; then
            read -r -p "Введите Jc: " AWG_JC; read -r -p "Введите Jmin: " AWG_JMIN; read -r -p "Введите Jmax: " AWG_JMAX; read -r -p "Введите S1: " AWG_S1; read -r -p "Введите S2: " AWG_S2; read -r -p "Введите H1: " AWG_H1; read -r -p "Введите H2: " AWG_H2; read -r -p "Введите H3: " AWG_H3; read -r -p "Введите H4: " AWG_H4
    ...
    if [ "$PROTOCOL_NAME" = 'AmneziaWG' ]; then
        uci set network.${INTERFACE_NAME}.awg_jc="$AWG_JC"; uci set network.${INTERFACE_NAME}.awg_jmin="$AWG_JMIN"; uci set network.${INTERFACE_NAME}.awg_jmax="$AWG_JMAX"; uci set network.${INTERFACE_NAME}.awg_s1="$AWG_S1"; uci set network.${INTERFACE_NAME}.awg_s2="$AWG_S2"; uci set network.${INTERFACE_NAME}.awg_h1="$AWG_H1"; uci set network.${INTERFACE_NAME}.awg_h2="$AWG_H2"; uci set network.${INTERFACE_NAME}.awg_h3="$AWG_H3"; uci set network.${INTERFACE_NAME}.awg_h4="$AWG_H4"
    fi
    ...
    if ! uci show firewall | grep -q "@zone.*name='${ZONE_NAME}'"; then
        info "Создание зоны firewall..."; uci add firewall zone; uci set firewall.@zone[-1].name="$ZONE_NAME"; uci set firewall.@zone[-1].network="$INTERFACE_NAME"; uci set firewall.@zone[-1].input='REJECT'; uci set firewall.@zone[-1].output='ACCEPT'; uci set firewall.@zone[-1].forward='REJECT'; uci set firewall.@zone[-1].masq='1'; uci set firewall.@zone[-1].mtu_fix='1'
    fi
    if ! uci show firewall | grep -q "@forwarding.*src='lan'.*dest='${ZONE_NAME}'"; then
        info "Настройка перенаправления трафика (forwarding)..."; uci add firewall forwarding; uci set firewall.@forwarding[-1].src='lan'; uci set firewall.@forwarding[-1].dest="$ZONE_NAME"
    fi
    uci commit firewall
    handler_network_restart
}
